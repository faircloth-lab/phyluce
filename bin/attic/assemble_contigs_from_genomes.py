...
from phyluce.helpers import (
    ...
    get_name,
    run_checks,
    snip_if_many_N_bases,
    get_uce_names_from_probes,
)
...
def get_args():
    ...
    parser.add_argument(
        "--flank", help="The flank length to add", default=500, type=int
    )
    parser.add_argument(
        "--db",
        action=FullPaths,
...
def create_probe_database(db, organisms, uces, genome=False):
    ...
    try:
        ...
        if e[0] == "table matches already exists":
            answer = input("Database already exists.  Overwrite [Y/n]? ")
            if answer == "Y" or "YES":
                os.remove(db)
        ...
        for table in ["matches", "match_map"]:
            try:
                ...
                if "duplicate column name" in e[0]:
                    ...
                    query = "UPDATE {0} SET {1} = NULL".format(
                        table, org.strip(" text")
                    )
                    c.execute(query)
    return conn, c
...
def main():
    ...
    uces = get_uce_names_from_probes(
        args.probes, regex=stripnum, repl="s", lower=True
    )
    ...
    for lz in glob.glob(os.path.join(args.lastz, "*")):
        ...
        for k, v in matches.items():
            ...
            loci_to_skip.extend(
                quality_control_matches(matches, probes, dupes, k, v, False)
            )
        # pdb.set_trace()
        # convert to set, to keep only uniques
        loci_to_skip = set(loci_to_skip)
        print(
            "\tSkipping {} bad (duplicate hit) loci...".format(
                len(loci_to_skip)
            )
        )
        ...
        for k, v in seqdict.items():
            ...
            if len(v) == 1:
                ...
                if result:
                    ...
                    record.sequence = snip_if_many_N_bases(
                        manyn, k, record.sequence, uce, verbose=False
                    )
                # change header
                ...
                if len(orient) == 1:
                    ...
                    record.sequence = str(aln.alignment_consensus)
                    record.identifier = ">Node_{0}_length_{1}_cov_1000".format(
                        count, len(record.sequence)
                    )
                    fout.write(record)
