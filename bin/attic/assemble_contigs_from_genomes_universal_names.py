...
from phyluce.helpers import FullPaths, is_dir, is_file, run_checks, snip_if_many_N_bases, get_name
...
def get_args():
    ...
    parser.add_argument(
            ...
            type=str,
            default=None,
            help="""Pattern for matching the lastz results""",
        )
...
def get_uce_names_from_probes(probes):
    loci = []
    for line in open(probes, 'rU'):
        if line.startswith('>'):
            loci.append(line.strip().lstrip('>').split(' ')[0].split('_')[0])
    # return unique set
    return set(loci)
...
def get_uce_num(header):
    """use own function vs. import from match_contigs_to_probes - we don't want lowercase"""
    return header.lstrip('>').split(' ')[0].split('_')[1]
...
def get_dupes(lastz_file, longfile=False):
    dupes = set()
    matches = get_dupe_matches(lastz_file, longfile)
...
def get_fasta_name_from_lastz_pth(lastz, fasta, pattern=None):
    if pattern:
        regex = re.compile(pattern)
        tempname = os.path.basename(lastz)
        result = regex.search(tempname).groups()[0]
    else:
        result = os.path.basename(os.path.splitext(lastz)[0])
    for ext in ['.fa', '.fasta', '.gz', '.fasta.gz', '.fa.gz']:
        tempname = os.path.join(fasta, result) + ext
        if os.path.isfile(tempname):
            break
        else:
            tempname = None
    if not tempname:
        raise ValueError("Cannot find the fasta file {}".format(result))
    return tempname
