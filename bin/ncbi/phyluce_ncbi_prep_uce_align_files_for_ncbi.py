...
from phyluce.helpers import is_dir, is_file, get_alignment_files, get_file_extensions, FullPaths
        ...
        for f in alignments:
            ...
            if uce not in locus_excludes:
                for seq in AlignIO.read(f, args.input_format):
                    # get species name
                    sp, species, partial, oldname = ncbi.get_species_name(seq.id, remap)
                    # skip taxa in exclude list
                    if species in taxon_excludes or oldname in taxon_excludes:
                        pass
                    else:
                        new_id = ncbi.get_new_identifier(species, uce, partial, counter, metadata, vouchers)
