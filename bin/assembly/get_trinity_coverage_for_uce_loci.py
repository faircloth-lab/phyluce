...
def get_args():
    ...
    parser.add_argument(
        ...
        type=is_file,
        action=FullPaths,
        help="""The path to the locus file generated by get_match_counts.py"""
    )
    parser.add_argument(
        "--sqlite",
        required=True,
        type=is_file,
        action=FullPaths,
        help="""The path to the database generated by match_contigs_to_probes.py"""
    )
    parser.add_argument(
        "--output-dir",
        required=True,
...
def setup_logging(level):
    log = logging.getLogger("Get UCE Coverage")
...
def create_per_locus_coverage_file(log, output_dir, assembly, organism, locus_map, locus_map_names):
    ...
    interval_list_pth = os.path.join(output_dir, "{}-UCE-matches-interval.list".format(organism))
    coverage_untrimmed = []
    coverage_trimmed = []
    trimmed_length = []
    count = 0
    with open(os.path.join(output_dir, "{}-UCE-per-contig-coverage.txt".format(organism)), 'w') as outf:
...
def create_per_base_coverage_file(log, output_dir, assembly, organism, locus_map, locus_map_names):
    log.info("Generating per-base coverage file for UCE loci.")
    with gzip.open(os.path.join(output_dir, "{}-UCE-per-base-coverage.txt.gz".format(organism)), 'w') as outf:
...
def main():
    ...
    if not os.path.isdir(args.output_dir):
        ...
        raise IOError("The directory {} already exists.  Please remove by hand.".format(args.output_dir))
    # get the input data
    log.info("Fetching input filenames")
    assemblies = glob.glob(os.path.join(args.assemblies, "*"))
    loci = get_match_count_loci(log, args.locus_file)
    ...
    for assembly in assemblies:
        ...
        locus_map = get_sqlite_loci_for_taxon(log, args.sqlite, cur, organism, loci)
        locus_map_names = set(locus_map.keys())
        #create_per_base_coverage_file(log, args.output_dir, assembly, organism, locus_map, locus_map_names)
        coverages_dict = create_per_locus_coverage_file(log, args.output_dir, assembly, organism, locus_map, locus_map_names)
        # pass the same intervals as targets and base - we don't care that much here about bait performance
        hs_metrics_file = picard_calculate_hs_metrics(log, organism, args.output_dir, reference, bam, coverages_dict["interval_list"], coverages_dict["interval_list"])
