...
def get_trimmed_coverage_from_output(log, sample, assembly_pth, coverage, assembler):
    ...
    overall_contigs = {}
    pbc = os.path.join(
        assembly_pth, "{}-TRIMMED-per-base-coverage.txt.gz".format(sample)
    )
    pcc = os.path.join(
        assembly_pth, "{}-TRIMMED-per-contig-coverage.txt".format(sample)
    )
    upcc = os.path.join(
        assembly_pth, "{}-UNTRIMMED-per-contig-coverage.txt".format(sample)
    )
                        ...
                        if previous_match is None or match_name == previous_match:
                            ...
                            if metadata["ending-mean-cov"] >= 5.0:
                                ...
                                for pos, line in contig_data.items():
                                    if (
                                        pos - 1 >= metadata["trim-start"]
                                        and pos - 1 < metadata["trim-end"]
                                    ):
    ...
    log.info(
        "\t{} contigs, mean coverage = {:.1f}, mean length = {:.1f}".format(
            overall_count, numpy.mean(overall_coverage), numpy.mean(overall_length)
        )
    )
...
def filter_screened_contigs_from_assembly(
    log, sample, assembly_pth, assembly, overall_contigs
):
    log.info("Filtering assembly FASTAs for coverage")
    keep = set(overall_contigs.keys())
    outf_name = os.path.join(assembly_pth, "{}-trinity-FILTERED.fasta".format(sample))
